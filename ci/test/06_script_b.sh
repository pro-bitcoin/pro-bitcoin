...
if [ "$RUN_UNIT_TESTS" = "true" ]; then
  DOCKER_EXEC ${TEST_RUNNER_ENV} DIR_UNIT_TEST_DATA=${DIR_UNIT_TEST_DATA} LD_LIBRARY_PATH=$DEPENDS_DIR/$HOST/lib \
      make $MAKEJOBS check VERBOSE=1 BOOST_TEST_REPORT_FORMAT="XML" BOOST_TEST_REPORT_SUFFIX="xml" BENCH_ARGS="'-output_json=$BASE_ROOT_DIR/ci/scratch/bench.json -output_csv=$BASE_ROOT_DIR/ci/scratch/bench.csv'"
fi
